...
import { useUpdatePost } from "@/hooks/usePostManagement";
...
export default function EditPostScreen() {
  ...
  useEffect(() => {
    if (post) {
      ...
      if (post.media_urls && post.media_urls.length > 0) {
        const mediaItems: PickedMedia[] = post.media_urls.map(url => ({
          uri: url,
          kind: "image" as const, // Default to image, can't determine type from URL
        }));
      ...
      // Set itinerary
      if (post.itinerary) {
        const matchingItinerary = myItineraries.find(it => String(it.id) === String(post.itinerary!.id));
        if (matchingItinerary) {
          setSelectedItinerary(matchingItinerary);
        }
      }
    }
  }, [post, myItineraries]);
  ...
  const handleSubmit = async () => {
    if (!post || !id) {
      showErrorToast("Lỗi", "Không tìm thấy bài viết");
      return;
    }
    ...
    await requireAuth(async () => {
      ...
      try {
        ...
        const newMediaItems = selectedMedia.filter(item => !item.uri.startsWith('http'));

        // Upload new media (local files that don't start with http)
        if (newMediaItems.length > 0) {
          ...
          for (let i = 0; i < newMediaItems.length; i++) {
            ...
            setUploadProgress(`Đang tải ${i + 1}/${newMediaItems.length}...`);
            ...
            try {
              ...
              showErrorToast(
                "Lỗi tải ảnh",
                `Không thể tải file ${i + 1}/${newMediaItems.length}. Vui lòng thử lại.`
              );
              return;
            }
          }
        ...
        // Combine existing remote URLs with newly uploaded URLs
        ...
        setUploadProgress("Đang cập nhật bài viết...");
        ...
        showErrorToast("Cập nhật thất bại", msg);
      } finally {
  ...
  // Show unauthorized error if not owner
  if (!isOwner) {
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.errorContainer}>
            ...
            <View style={styles.itineraryBlockHeader}>
              ...
              {selectedItinerary ? (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <Text style={styles.itineraryRemoveLink}>Gỡ</Text>
                </TouchableOpacity>
              ) : null}
            </View>
